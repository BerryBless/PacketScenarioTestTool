...
void EchoServer::OnClientJoin(WCHAR* ipStr, DWORD ip, USHORT port, SESSION_ID sessionID) {
...
void EchoServer::OnClientLeave(SESSION_ID sessionID) {
	int idx = sessionID >> (8 * 6);
	//if (idx == 0) CRASH();
}

void EchoServer::OnRecv(SESSION_ID sessionID, Packet* pPacket) {

	EchoProc(sessionID, pPacket);
}

void EchoServer::OnSend(SESSION_ID sessionID) {
}

void EchoServer::OnError(int errorcode, const WCHAR* log) {
	//wprintf_s(L"%d :: %s", errorcode, log);
}

void EchoServer::OnTimeout(SESSION_ID sessionID) {
	printf_s("OnTimeout :: %lld\n", sessionID);
	DisconnectSession(sessionID);
}

bool EchoServer::BeginServer(u_long IP, u_short port, BYTE workerThreadCount, BYTE maxRunThreadCount, BOOL nagle, u_short maxConnection) {
...
void EchoServer::PrintMonitor(FILE* outFP) {
...
void EchoServer::PrintAverage(FILE* outFP) {
	fwprintf_s(outFP, L"\n----------------------------AVERAGE-----------------------------------\n");
...
void EchoServer::PrintFileMonitor() {
...
void EchoServer::AverageMonitor(MoniteringInfo monitor) {
...
void EchoServer::EchoProc(SESSION_ID sessionID, Packet* pPacket) {
	PRO_BEGIN(L"Content_Send");
	SendPacket(sessionID, pPacket);
	PRO_END(L"Content_Send");
}

void EchoServer::LockMap() {
}

void EchoServer::UnlockMap() {
}
